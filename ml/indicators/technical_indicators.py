...
class IndianMarketMetricsProvider:
    ...
    def get_basic_market_metrics(self, data: pd.DataFrame, prefetch: Dict[str, pd.DataFrame] | None = None) -> Dict[str, float]:
        ...
        """
        try:
            # Instantiate IndianMarketMetricsProvider inside the method
            market_metrics_provider_instance = IndianMarketMetricsProvider()
            ...
            if prefetch and isinstance(prefetch.get("NIFTY_50"), pd.DataFrame):
                ...
                nifty_data = market_metrics_provider_instance.get_nifty_50_data(period=365)
            ...
            if nifty_data is not None and len(nifty_data) > 0:
                ...
                if len(common_dates) > 30:
                    ...
                    beta = market_metrics_provider_instance.calculate_beta(stock_returns, market_returns)
                    correlation = market_metrics_provider_instance.calculate_correlation(stock_returns, market_returns)
                else:
                    beta = 1.0
                    correlation = 0.0
            else:
            ...
            # Get risk-free rate
            risk_free_rate = market_metrics_provider_instance.get_risk_free_rate()
    ...
    def get_enhanced_market_metrics(self, data: pd.DataFrame, stock_symbol: str, prefetch: Dict[str, pd.DataFrame] | None = None) -> Dict[str, float]:
        ...
        try:
            ...
            sector = market_metrics_provider_instance.sector_classifier.get_stock_sector(stock_symbol)
            ...
            if prefetch and isinstance(prefetch.get("NIFTY_50"), pd.DataFrame):
                ...
                nifty_data = market_metrics_provider_instance.get_nifty_50_data(365)
            if nifty_data is not None and len(nifty_data) > 30:
                market_returns = nifty_data['close'].pct_change().dropna()
                market_beta = market_metrics_provider_instance.calculate_beta(stock_returns, market_returns)
                market_correlation = market_metrics_provider_instance.calculate_correlation(stock_returns, market_returns)
                ...
                market_sharpe = (market_returns.mean() * 252 - market_metrics_provider_instance.get_risk_free_rate()) / market_volatility if market_volatility > 0 else 0
            else:
            ...
            if sector:
                sector_data = market_metrics_provider_instance.get_sector_index_data(sector, 365)
                if sector_data is not None and len(sector_data) > 30:
                    sector_returns = sector_data['close'].pct_change().dropna()
                    sector_beta = market_metrics_provider_instance.calculate_beta(stock_returns, sector_returns)
                    sector_correlation = market_metrics_provider_instance.calculate_correlation(stock_returns, sector_returns)
                    
                    # Calculate sector performance metrics
                    sector_cumulative_return = (1 + sector_returns).prod() - 1
                    sector_volatility = sector_returns.std() * np.sqrt(252)
                    sector_sharpe = (sector_returns.mean() * 252 - market_metrics_provider_instance.get_risk_free_rate()) / sector_volatility if sector_volatility > 0 else 0
            ...
            if prefetch and isinstance(prefetch.get("INDIA_VIX"), pd.DataFrame):
                ...
                vix_data = market_metrics_provider_instance.get_india_vix_data(30)
            ...
            return {
                ...
                "data_freshness": "real_time" if market_metrics_provider_instance._is_market_open() else "last_close",
                "market_status": "open" if market_metrics_provider_instance._is_market_open() else "closed"
            }
            
        except Exception as e:
            logging.error(f"Error calculating enhanced market metrics for {stock_symbol}: {e}")
            return market_metrics_provider_instance._get_default_market_metrics(stock_symbol)
