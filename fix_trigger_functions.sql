-- =====================================================
-- FIX TRIGGER FUNCTIONS - Resolve Column Reference Ambiguity
-- =====================================================

-- Drop the existing trigger first
DROP TRIGGER IF EXISTS extract_analysis_data_trigger ON stock_analyses;

-- Fix the extract_technical_indicators function with proper column qualification
...
BEGIN
    -- Get analysis data using the new column name with proper qualification
    SELECT sa.analysis_data_json INTO analysis_data 
    ...
    DELETE FROM technical_indicators WHERE technical_indicators.analysis_id = extract_technical_indicators.analysis_id;
...
-- Fix the extract_sector_benchmarking function
...
BEGIN
    ...
    DELETE FROM sector_benchmarking WHERE sector_benchmarking.analysis_id = extract_sector_benchmarking.analysis_id;
...
-- Fix the extract_pattern_recognition function
...
BEGIN
    ...
    DELETE FROM pattern_recognition WHERE pattern_recognition.analysis_id = extract_pattern_recognition.analysis_id;
...
-- Fix the extract_trading_levels function
...
BEGIN
    ...
    DELETE FROM trading_levels WHERE trading_levels.analysis_id = extract_trading_levels.analysis_id;
...
-- Fix the extract_volume_analysis function
...
BEGIN
    ...
    DELETE FROM volume_analysis WHERE volume_analysis.analysis_id = extract_volume_analysis.analysis_id;
...
-- Verify the trigger is recreated
SELECT 
    trigger_name,
    event_manipulation,
    action_timing,
